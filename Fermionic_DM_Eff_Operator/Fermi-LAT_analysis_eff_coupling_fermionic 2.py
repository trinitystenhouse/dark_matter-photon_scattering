...
def plot_tau_grid(
    *,
    ...
    if float(tau_needed) > 0.0:
        ...
        # Overlap region = EFT-valid AND testable (tau >= tau_needed).
        # Use a boolean mask for hatching + an explicit white outline.
        overlap = (
        ...
        if has_overlap:
            # Hatch fill (no facecolor), but force hatch/edge color to white.
            cf = axG.contourf(
                X,
                Y,
            ...
            # White outline around the overlap region.
            axG.contour(
                X,
                Y,
                overlap.astype(float),
