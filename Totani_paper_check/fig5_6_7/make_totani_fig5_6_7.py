...
def main():
    ap = argparse.ArgumentParser(description="Reproduce Totani Figs.5-7 using saved MCMC outputs.")
    ap.add_argument(
        "--mcmc-dir",
    ...
    ap.add_argument("--fig", type=str, default="fig5")
    ap.add_argument(
        "--out-txt",
        default=None,
    ...
    if args.outdir is None:
        args.outdir = os.path.join(os.path.dirname(__file__), f"plots_{args.fig}")
    os.makedirs(args.outdir, exist_ok=True)

    if args.out_txt is None:
        args.out_txt = os.path.join(os.path.dirname(__file__), f"mcmc_coefficients_{args.fig}.txt")
    ...
    if args.fig == "fig5":
        labels = ["gas", "iso", "ps", "loopA", "loopB", "ics", "nfw_NFW_g1_rho2.5_rs21_R08_rvir402_ns2048_normpole_pheno", "fb_pos", "fb_neg"]

    elif args.fig == "fig6":
        labels = ["gas", "iso", "ps", "loopA", "loopB", "ics", "nfw_NFW_g1_rho2_rs21_R08_rvir402_ns2048_normpole_pheno", "fb_pos", "fb_neg"]

    elif args.fig == "fig7":
        labels = ["gas", "iso", "ps", "loopA", "loopB", "ics", "nfw_NFW_g1_rho1_rs21_R08_rvir402_ns2048_normpole_pheno", "fb_pos", "fb_neg"]

    else:
        raise ValueError("--fig must be one of: fig5, fig6, fig7")
