...
def main():
    ...
    _, hdr_cnt, Emin, Emax, Ectr_mev, _dE_unused = read_counts_and_ebounds(args.counts)
    nE = int(Ectr_mev.size)

    w_tgt = WCS(hdr_cnt).celestial
    ny, nx = hdr_cnt["NAXIS2"], hdr_cnt["NAXIS1"]

    dE_mev = (Emax - Emin)  # MeV
    ...
    m = (E_iso > 0) & (I_iso > 0)
    I_ctr = np.exp(np.interp(np.log(Ectr_mev), np.log(E_iso[m]), np.log(I_iso[m])))
    ...
    iso_E2dnde = iso_dnde * (Ectr_mev[:, None, None] ** 2)

    # Expected counts per bin:
    # mu = (dN/dE) * exposure * Ω_pix * ΔE
    mu_iso = iso_dnde * expo * omega[None, :, :] * dE_mev[:, None, None]

    k = 2  # ~3.8 GeV in my binning
    conv_k = expo[k] * omega * dE_mev[k]

    print("Ectr(MeV) =", Ectr_mev[k])
